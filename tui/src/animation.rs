    ...
    pub fn load(path: impl AsRef<Path>) -> Result<Self, AnimationError> {
        ...
        for (line_no, line) in text.lines().enumerate() {
            ...
            let raw: RawFrame = match serde_json::from_str(line) {
                Ok(v) => v,
                Err(e) => {
                    eprintln!("sub-zero-tui: skipping line {} of {}: {}", line_no + 1, path.display(), e);
                    continue;
                }
            };
        ...
        let total_duration_ms = frames.iter()
            .map(|f| if f.delay_ms == 0 { 80 } else { f.delay_ms as u64 })
            .sum::<u64>();
        ...
        Ok(Self { frames, width, height, total_duration_ms, audio_path })
    }
}
...
fn decode_frame(raw: RawFrame) -> Frame {
    ...
    for (_row_i, row) in raw.rows.into_iter().enumerate() {
        let mut col = 0u16;
        for cell_v in row {
            if col >= raw.width {
                break;
        ...
        while col < raw.width {
            cells.push(Cell { ch: ' ', r: 0, g: 0, b: 0 });
            col += 1;
        }
    }
...
fn decode_cell(v: &serde_json::Value) -> Cell {
    ...
    let arr = match v.as_array() {
        Some(a) => a,
        None => return Cell { ch: ' ', r: 255, g: 255, b: 255 },
    };
    let ch = arr.get(0)
        .and_then(|s| s.as_str())
        .and_then(|s| s.chars().next())
        .unwrap_or(' ');
    let r = arr.get(1).and_then(|v| v.as_u64()).map(|v| v as u8).unwrap_or(255);
    let g = arr.get(2).and_then(|v| v.as_u64()).map(|v| v as u8).unwrap_or(255);
    let b = arr.get(3).and_then(|v| v.as_u64()).map(|v| v as u8).unwrap_or(255);
    Cell { ch, r, g, b }
}
